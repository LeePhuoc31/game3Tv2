...
public class onePlayer15x15 extends JFrame implements ActionListener {
	boolean win = false;
	String icon;
	...
	private JButton bt[] = new JButton[225];
	private int a[] = new int[225];
	...
	public onePlayer15x15(String s, int Point1, int Point2) {
		super(s);
		taoGUI();
		...
		File f = new File("src\\TicTacToe\\data115.txt");
	...
	public void addP1() {
		...
		p1.setLayout(new GridLayout(15, 15, 0, 0));
		for (int i = 0; i < 225; i++) {
			bt[i] = new JButton(" ");
			p1.add(bt[i]);
			...
			bt[i].setPreferredSize(new Dimension(8, 8));
			bt[i].setForeground(Color.red);
			bt[i].setBackground(Color.white);
			bt[i].setFocusPainted(false);
			bt[i].setFont(new Font("Forte", Font.PLAIN, 8));
			a[i] = 1;
		}
	}
	...
	public void taoGUI() {
		setIconImage(Toolkit.getDefaultToolkit().getImage(onePlayer15x15.class.getResource("/images/icon.png")));
...
//				btnReplay.setEnabled(false);
	...
	public void xoaFile() {
		File myObj = new File("src\\TicTacToe\\data115.txt");
		myObj.delete();
	}
...
//			btnRedo.setEnabled(true);
	...
	public void writeData() {
		...
		try {
			oos = new ObjectOutputStream(new FileOutputStream("src\\TicTacToe\\data115.txt"));
...
//                System.out.println("Lỗi đóng file");
	...
	public void readData() {
		...
		try {
			ois = new ObjectInputStream(new FileInputStream("src\\TicTacToe\\data115.txt"));
...
//		}
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getActionCommand() == btnReplay.getActionCommand()) {
			onePlayer15x15 k = new onePlayer15x15("", point1, point2);
			if (Math.random() >= 0.5) {
				k.timer.start();
				k.icon = "O";
			} else
	...
	public static void main(String[] args) {
		onePlayer15x15 k = new onePlayer15x15("", 0, 0);
		if (Math.random() >= 0.5) {
			k.timer.start();
			k.icon = "O";
		} else
